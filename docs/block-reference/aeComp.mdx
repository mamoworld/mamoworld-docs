...
Returns one of the attributes of the given composition, like its active camera, number of layers or motion blur settings. For a description of all available attributes see [Set Attribute of Comp](#ae_comp_set_attribute). Note that compositions are also project items. To access properties that are available on all project items (like the project bin where it is located) you need to use the block [Get Attribute of Project Item](aeProjectItems#ae_item_get_attribute) (which also accepts comps as arguments).
...
Note that compositions are also project items. To set properties that are available on all project items (like the project bin where it is located) you need to use the block [Set Attribute of Project Item](aeProjectItems#ae_item_set_attribute) (which also accepts comps as arguments).
...
To do all calculations in full resolution, you can either parse `1` or a list `[1,1]`. A value of `4` sets the resolution to *Quarter*.
