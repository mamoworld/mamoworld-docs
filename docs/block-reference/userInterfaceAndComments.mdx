...
By default, it shows a user interface which demonstrates all the available elements:
...
If you put a [File block](files#block_extendscript_file) into the `preselected file` parameter, the dialog will choose this file by default, but still allow the user to choose another.
