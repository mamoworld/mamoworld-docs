...
### dropdown list entries (write only)

A [list](/block-reference/lists.mdx) (array) of texts. This attribute can only be set for one property, namely the **Dropdown Menu Control** effect. It allows to set the labels for the dropdown options, in the same way as if you would click the *Edit* button of this effect.

<RetinaImg src="/img/blockOther/ae_property_set_attribute_dropdown_list_entries.png" alt="Setting dropdown list entries of the Dropdown Menu Control"/>

To insert an separator in the list, add an empty string to the like (or alternatively the text `(-`). The `|` symbol is not allowed in label names and will be removed.
If you try to set the dropdown list entries for other properties than the *Dropdown Menu Control* effect, a warning is printed on the console and the request is ignored.

:::danger Matchname Change
...
In [this example](https://www.dropbox.com/s/fj2wxz56b1tgk9u/Create%20Dropdown%20Effect%2C%20Change%20Choices%20and%20Choose%20One.xml?dl=0) we first create a dropdown list on the only selected layer. Then we set its dropdown list options, and after that change the name of the dropdown and choose the fourth of the new list options:

<RetinaImg src="/img/blockOther/ae_property_set_attribute_dropdown_list_entries_example.png" alt="Example: create dropdown effect, change its labels and pick one of them."/>
...
## Delete Property {#ae_property_remove}

<RetinaImg src="/img/blockScreenshots/AeLayerProperties/ae_property_remove.png" alt="Block ae_property_remove"/>


Deletes the given property (or [list](/block-reference/lists.mdx) (array) of properties). If the property is a group, all child properties are removed as well. Note that not all properties can be deleted - you can delete an effect, for example, but you cannot delete an individual parameter of an effect or the transform group.
...
See also the [Project Item Exists](aeProjectItems.mdx#ae_item_exists) block.
