...
This block creates a single keyframe - you just need to specify the [property](aeLayerProperties.mdx) on which the keyframe should be created, the [time](/block-reference/aeTime.mdx) and the [value](aeLayerProperties#ae_property_value_json) for the keyframe.
