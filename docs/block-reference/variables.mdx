...
This block is only useful for variables whose value is a number. The block allows you to increase the value of a variable (or decrease it, if you provide a negative number). The same can also be done with the [Set Variable block](#variables_set), but with the Change Variable block the code is a bit more compact. 
