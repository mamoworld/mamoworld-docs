...
Whenever you read or write a spreadsheet file, make sure it has either the file extension .csv or .xml. Based on this file extension, Automation Blocks will decide whether it treats the file as csv or Excel XML file.
...
If the spreadsheet file already exists, this block will append a new row/column to it. If you want to start a new file, just [delete the existing file](files#block_extendscript_delete_file) before this block. 
...
With the optional `from row` and `to row` parameters you can limit the range of rows that is read. If the first row of your spreadsheet contains a title, for example, that you want to skip, just set `from row` to `2` (Cell indices start at 1 for the first row).

By default, from each row the first two cells are read (cell A and cell B), but if you click on the cog wheel icon in the top left corner of the block, you can add more cells.

The blocks that you nest inside this block are executed once for each row of the spreadsheet. During that execution, the content of each cell in that row is stored in a [variable](variables.mdx) (`my cell 1`, `my cell 2` etc.) so that you can easily access the content of these cells.
