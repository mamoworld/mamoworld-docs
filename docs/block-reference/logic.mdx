...
The [End of File](filesText#extendscript_text_file_eof) block returns `true` if the script has read the entire contents of the text file already. If you plug it into a *Not* block, it returns `true` if there is still content left to read.
...
You can also use Null directly in a logic test (as the condition of an [if block](logic#controls_if), for example). In a logic test, Null will behave like `false` whereas any other object behaves like `true`. For example, see the [Ternary Test](logic#logic_ternary) below.
