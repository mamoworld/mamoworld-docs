...
**import multi-layer files as** allows you to import those as `footage`, `comp` or `comp (cropped layers)`. These are exactly the same options as those provided by Ae's normal import dialog.

**import numbered images as** allows you to import images as `still image` or `image sequence`. Note that images, which have no number at the end of their file name, are always imported as `still image`. So, if you set this option to `image sequence`, a file *myImage.png* will still be imported as a still image, but an image *myImage000.png* or *myImage004.png* will be treated as part of an image sequence.
