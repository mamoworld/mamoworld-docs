...
In German Pr, for  example, "Scale" is named "Skalieren" and "Motion" is "Bewegung". To avoid issues, I recommend to switch the UI language of Pr to English. You can also create scripts using names in other languages (but then your scripts will only be compatible with that language). All sample scripts included in Automation Blocks rely on the English names.
