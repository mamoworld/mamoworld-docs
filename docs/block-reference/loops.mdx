...
This loop just executes the blocks you plug inside it as many times as you want. It is the simplest loop and therefore useful to understand the concept of loop blocks. However, in practice you will use the [For Loop](loops#controls_forcontrols_for) much more often, which is very similar, but more powerful.
...
By the way, if you're wondering what the [Tinker File Path block](filesNameAndPath#string_tinker_path) here does: from the full path (say `C:\projects\myProject\footage`), it just takes the name (like `footage` in our example) and puts a `\` in front of it to indicate that we want to import it into a bin in the root bin of the project (`\footage` denotes a bin "footage" in the root bin, whereas `footage` denotes a bin with name "footage" that could be anywhere - so Automation Blocks would search your project to see if it can find a bin of that name).
