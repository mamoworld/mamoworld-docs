...
The alpha mode of After Effects' Interpret Footage dialog. If you get this value, it will be one of the following [texts](text.mdx)
...
You can get and set this attribute for items of [type](#ae_item_set_attribute_type) `FootageNoSolidItem`, for other items it will produce an error.
...
You can get and set this attribute for items of [type](#ae_item_set_attribute_type) `FootageNoSolidItem` and `CompItem`, for other items it will produce an error.
...
Same as the `frame rate` attribute, unless you change the frame rate of a footage item using the "conform to frame rate" option of Ae's Interpret Footage dialog (or by setting the `frame rate` attribute with Automation Blocks).
In that case, the `frame rate (native)` still contains the original frame rate of the footage file, whereas `frame rate` contains the conformed frame rate.

### has alpha (read-only)

A[logic value](/block-reference/logic.mdx) `true` or `false` indicating if the item has an alpha channel. This value is only relevant for items of [type](#ae_item_set_attribute_type) `FootageNoSolidItem`. For comps it is always `true` and for bins and solids it is always `false`.
...
A[logic value](/block-reference/logic.mdx) `true` or `false` which corresponds to the "Invert Alpha" option of Ae's Interpret Footage dialog.
You can get and set this attribute for items of [type](#ae_item_set_attribute_type) `FootageNoSolidItem`, for other items it will produce an error.


### is still

A[logic value](/block-reference/logic.mdx) `true` or `false` indicating if this item is a still image. This value is only relevant for items of [type](#ae_item_set_attribute_type) `FootageNoSolidItem`.It is always false for comps, bins and solid items.
...
A number corresponding to the loop option of Ae's Interpret Footage dialog.
You can set this attribute for items of [type](#ae_item_set_attribute_type) `FootageNoSolidItem`, for other items it will produce an error.
...
See also the [Property Exists](aeLayerProperties.mdx#ae_property_exists) block.
