...
You can create, copy, move or delete layers, apply effects of presets to them and change their attributes, for example.
...
The biggest different between the first two is that when you click the *refresh* icon of the layer block, it will fill all its text fields automatically based on the layer that is currently selected in the Timeline panel. With the "root composition", in addition the text is split into one component for the layer and one for the containing comp, such that you can manipulate each of them individually.
...
To specify a layer in the active composition, you can simply skip the part of the path, which specifies the composition itself. Here are three different ways to choose the layer "White Solid 1" in the active composition:
...
If you skip the comp name, you can usually also skip the `::` separator. However, you can also keep the `::` before the layer name to tell Automation Blocks explicitly that this text describes a layer. Let's take a look at these examples:

<RetinaImg src="/img/blockOther/access_layer_example_active_comp_colons.png" alt="layer path vs project item path"/>

Note that the first one uses the block [Get Attribute of Layer](aeLayer.mdx#ae_layer_get_attribute), whereas the second and third one use the block [Get Attribute of Project Item](aeProjectItems.mdx#ae_item_get_attribute). Hence, the first example retrieves the label color name of the layer with name `my video` (of the active comp), whereas the second example retieves the label color of a project item with name `my video`. In the third example, we put the colons in front of the name to make clear that this should actually be a layer name. Hence, Automation Blocks searches for the layer with name `my video` (in the active composition) and because the block expects a project item as argument, it then attempts to convert the layer to a project item. For details about this, see the section about [accessing a project item using a layer or property](aeProjectItems#accessProjectItem).

### (Not) Unique Layer Names

We recommend that you make sure that each layer of a composition has a unique name. But if you have multiple layers with name `White Solid 1`, then `Comp 1::White Solid 1` will refer to the first one of those. If you want to access another one, you can append a number in square brackets to the name. To obtain the second layer with name `White Solid 1` you can use `White Solid 1[2]`, for example.
...
This block is build into many other blocks as parameter - in every situation where you need to choose a layer, you will most likely find this block. Picking a particular layer is very easy: Just select the layer and click the *refresh* icon of this block.
...
This block returns the only selected layer, if exactly one layer is selected and stops with an error otherwise.

This is very useful, if you want to write a script, which assumes that the user has selected a single layer and don't want to worry about dealing with a [List of Layers](#ae_layer_list) and a check if that list has length 1.
...
This block is a [loop](loops.mdx) which allows to do something with all layers, or all selected layers of a composition. You can also select only a particular type of layer that should be considered.
...
The block executes all the blocks which you plug inside of it many times - namely one time for each of the layers. And while these blocks are executed, the variable **my layer** always contains that respective layer. In other words, you can use **my layer** as a placeholder for the layers over which the block loops.
...
The new layer is always inserted as first layer of the comp (you can change its [index](#ae_layer_set_attribute_index) with the [Set Attribute of Layer](#ae_layer_set_attribute) block).
...
If you want to create a new footage layer, you need to  specify the name for the new layer and the project item, which should be used as the footage of the layer.
Note that the project item can also be a composition; in that case the new layer will be precomp. 

**Example**

This script first imports the file `test.mp4` from the desktop into the After Effects project and then inserts it as new layer into the active composition. Note that the [Import File](aeProject.mdx#block_ae_import_file) block saves the project item of the imported file in the [variable](variables.mdx) `project item var` and then the *New Layer* block uses this variable to create a layer with that new project item.
...
If you want to create a shape layer, the only parameter you need to specify is the *name* of the layer. The block will create an empty shape layer for you and you need to use the [Add Effect/Mask/Shape](#ae_apply_effect) block, to add shapes to that layer. Here is an example, which first creates the layer and then adds an ellipse path with a red fill to it:
...
In the first row of the block, you can choose the kind of effect/mask/shape you want to create as well as the [Layer](aeLayer.mdx#accessLayer) on which it should be created. The block also creates a variable `my variable` and assigns the [Layer Property](aeLayerProperties.mdx#ae_layer_properties_access) of the newly created effect/mask/layer to it. That way, you can easily access it other blocks of your script, after it has been created.

### Adding Effects 

To create an effect, use the drop downs in the first row of the block to first choose the effect category and then the effect in that category. Once you've chosen an effect, the block contents updates and shows exactly the same parameters as the respective effect has in the Effect Controls panel. Here we've chosen the "Gaussian Blur" effect from the category "Blur & Sharpen":
...
If you choose to create a mask (using the first dropdown of the block), the blocks shows you the following parameters:
...
If you choose to create a shape (using the first dropdown of the block), in a second dropdown you can choose the type of shape, or shape modifier you want to create.

Exactly as for effects or masks, the block then shows the parameters of the respective shape type (or shape modifier) such that you can adjust them as needed.

**Creating Shapes inside Groups**

Whereas effects and masks are always applied to a layer, shapes can be added either to the top level (i.e to the "Contents" group) or to sub-groups. If you want to use a subgroup, you need to drag a [Layer Property block](aeLayerProperties.mdx##ae_property_path) on top of the layer input (since groups are also layer properties). In that block you can then connect the subgroup, into which the new shape (or shape modifier) should be inserted.

This video shows how to create a rectangle path first on the top level and then inside a sub-group.
...
After Effects saves presets in `.ffx` files. If you click on the _Open_ icon of the _Preset File_ it shows the folder where After Effects saves those files by default, such that you can pick the one you want to use.
...
To avoid that issue, you can include the preset file as embedded file into your block script as follows:
...
Now your script uses an embedded file whose contents is saved with your blockscript such that it is always available when you run the block script, no matter if the preset file is installed on the machine or not.
...
For text layers only - a [logic value](logic.mdx) which is `true` if the `3D per character` option is enabled or the character animators.
...
A [logic value](logic.mdx) value, which is `true`, if this layer is an environment layer. Setting this value to `true` or `false` has the same effect as if you right-click on the layer and enable/diable the option *Environement Layer* in the context menu.

### eyeball icon

<RetinaImg src="/img/blockOther/layer_attribute_eyeball_icon.png" alt="After Effects Attribute Eyeball Icon"/>

A [logic value](logic.mdx) which you can set to `true` or `false` to turn the eyeball icon of the layer. Set it to `false` to hide the layer or to `true`to make it visible.
...
If you want to set this value, you can either set it to one of those texts or one of the numbers 1-3. Upper/lower case of the texts is ignored, and instead of underscores you can also use spaces. Hence, setting this value to `NO_FRAME_BLEND`, to `1`, or to `no frame blend` gives exactly the same result.

### guide layer

A [logic value](logic.mdx) `true` or `false` indicating if the *guide layer* option is enabled for this layer. Setting this value to `true` has the same effects as if you right-click on the layer and activate the *Guide Layer* option.
...
*negative number*
If you set this value to a negative number, the index is counted from the end of the composition. Hence, if you set the value to `-1`, it will become the last layer of the comp and if you set it to `-2`, it will become the last but one.
...
If this value is `false`, the layer name is identical to the name of the corresponding project item **and when the name of the project item is changed, the layer name changes with it**. Changing layer names can cause a lot of trouble, since Automation Blocks accesses layers by their name. Hence, a layer that is bound to a [variable](variables.mdx), cannot be found by Automation Blocks anymore, if you change the layer name.
...
To prevent layer name changes when a project item is renamed or when the text of a text layer is changed, you can set this attribute to `true`. If you do that, Automation Blocks will manually rename the layer (actuall to its current name, such that the name does not really change). After this renaming operation, the layer name will be independent of the project item's name and the layer's source text.

Note that you cannot set this property to `false`. In other words, this is a one way street: Whenever a layer has been renamed once, it cannot go back to its default naming.
...
A [logic value](logic.mdx) which corresponds to the *locked* switch of the layer. Note that also scripts can only change layers if they are unlocked.
...
Setting this value to true, has the same effect as if you would right-click on the layer and choose *"Time->Enable Time Remapping"* in the context menu.
...
If you want to set this value, you can either set it to one of those texts or one of the numbers 1-5. Upper/lower case of the texts is ignored, and instead of underscores you can also use spaces. Hence, setting this value to `NO_TRACK_MATTE`, to `1`, or to `no track matte` gives exactly the same result.
...
The duplicate is saved in the [variable](variables.mdx) `new layer var`, such that you can easily access the layer in subsequent blocks of your script. For an example how this works, see the block [Copy Layer](#ae_layer_copy_to_comp).
...
Copies a single [Layer](aeLayer.mdx#accessLayer) to a different [Composition](aeComp.mdx#accessComp). The layer in the new composition is saved in the [variable](variables.mdx) `new layer var`, such that you can easily access the layer in subsequent blocks of your script.

The layer is inserted above the first currently selected layer or as first layer of the comp if nothing is selected.
...
Note how we use the [variable](variables.mdx) `new layer var` that is created by the first block in the second and third block to access the newly created layer.
...
Deletes the given [Layer(s)](#accessLayer). You can give this block either a single layer as argument, or also a [list](lists.mdx) (array) of layers do delete all of them.
...
Selects or deselects the given [Layer(s)](#accessLayer). You can give this block either a single layer as argument, or also a [list](lists.mdx) (array) of layers do (de)select all of them.
...
Precomposes the given layers. You can give either a single layer or an entire [list](lists.mdx) (array) of layers as argument.

If you give an entire list of layers, they are all precomposed together into a single comp. Also, in that case the *"move all attributes"* option is ignored, since it always needs to be *true*, if multiple layers are precomposed.
...
Adjusts the precomp duration such that it starts at the eariest in point and ends at the latest out point of the precomposed layers.

**crop to source rect**

Here you can pass a [list](lists.mdx) (array) with values `[top, left, width, height]` to crop the precomp to that area of the original comp. If you plug the [Get Source Rect of Layer(s)](#ae_layer_source_rect) block here, for example, the precomp will be cropped to the area covered by these layers (make sure to get the value in *Comp Space*, not *Layer Space*).
...
This block returns a [list](lists.mdx) (array) with values `[left, top, width, height]`, which describe the area covered by the layer. This is similar to the layer attributes [width](#ae_layer_set_attribute_width), [height](#ae_layer_set_attribute_height) and [size](#ae_layer_set_attribute_size), but with two major differences:
...
The source rect can be returned in comp space or in layer space. "Comp space" means that the positions are measured as pixel values of the composition (like the X/Y coordinates shown in the Info panel, when you hover with the mouse over the Composition panel). "Layer space" means, that the coordinates are given measured in the coordinates of the individual layer (like the X/Y coordinates shown in the Info panel, when you hover with the mouse over a Layer panel instead of the Composition panel). The source rect in layer space only changes, if you actually change the content of the layer, whereas the source rect in comp space also changes, when you move, scale or rotate the layer, for example.

In this example, the source rect of the text in layer space is visualized in red, and the source rect in comp space in dark grey:
<RetinaImg
