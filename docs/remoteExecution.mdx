...
Automation Blocks tools can be launchen in all kinds of ways, using external devices, your phone, keyboard shortcuts or other software running on your machine. There are two methods to do that:

- *HTTP Requests* are great to lauch AB scripts from external devices like [Stream Deck](#streamdeck), from automation software like [Auto Hotkey](#autohotkey) or [Keyboard Maestro](#keyboardmaestro) and from your phone using [Touch Portal](#touchportal).
