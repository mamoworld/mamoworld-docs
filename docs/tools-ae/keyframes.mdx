...
For a quick cut & paste of keyframes, it is too cumbersome to save a preset each time. Hence, with this script you can cut & paste keyframes with the the same [automatic adjustment of keyframes](#AutomaticKeyframeAdjustment) that our keyframe preset tools are using, but without the need to [save a preset](#SavePreset).
